...
from .serializers import (
    ...
    ContatoNormal,
    ContatoDeLoja,
    ListaProdutosCategoria, CategoriaParaListaSerializer, # <-- Adicionado
    ProdutoSerializer, ProdutoLiteListSerializer, # <-- Adicionado
)
...
class CategoriaViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Endpoint para visualizar categorias.
    - GET /categorias/ -> Lista simples de categorias.
    - GET /categorias/{id}/ -> Detalhes de uma categoria e seus produtos.
    """
    queryset = Categoria.objects.all()
    permission_classes = [HasAPIKey]

    def get_serializer_class(self):
        # Se a ação for 'retrieve' (buscar um), usa o serializer completo.
        if self.action == 'retrieve':
            return ListaProdutosCategoria
        # Para qualquer outra ação (como 'list'), usa o serializer leve.
        return CategoriaParaListaSerializer


class ProdutoViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Endpoint para visualizar produtos.
    - GET /produtos/ -> Lista simples de produtos.
    - GET /produtos/{id}/ -> Detalhes completos de um produto.
    """
    ...
    def get_serializer_class(self):
        ...
        # Para qualquer outra ação (como 'list'), usa o serializer super leve.
        return ProdutoLiteListSerializer