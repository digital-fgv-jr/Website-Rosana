...
## SERIALIZERS PARA LISTAGEM

class ProdutoLiteListSerializer(serializers.ModelSerializer):
    """
    Serializer super leve para listar produtos, mostrando apenas nome e a imagem principal.
    """
    imagem_principal_url = serializers.SerializerMethodField()

    class Meta:
        model = Produto
        fields = ['id', 'nome', 'preco', 'imagem_principal_url']
...
class CategoriaParaListaSerializer(serializers.ModelSerializer):
    """
    Serializer para a lista de categorias, mostrando apenas o nome e o ID da loja.
    """
    # Em vez de um objeto aninhado, mostramos apenas o ID da loja.
    loja = serializers.PrimaryKeyRelatedField(read_only=True)
    
    class Meta:
        model = Categoria
        fields = ['id', 'nome_categoria', 'loja']