...
export default function Contato() {
  ...
  const handleSubmit = (e) => {
    ...
    setSent(true);
    e.target.reset();
    // TODO: integrar com backend/email service futuramente
  };

  // === Dados fixos da Rosana ===
  const WHATS_NUM_E164 = "5521984744189";
  const WHATS_DISPLAY = "+55 (21) 98474-4189";
  const WHATS_MSG = "Olá! Gostaria de falar com a Rô Jewellery 🙂";
  const WHATS_HREF = `https://wa.me/${WHATS_NUM_E164}?text=${encodeURIComponent(WHATS_MSG)}`;
            ...
            <ul className="info-list">
              ...
              <li>
                ...
                <div>
                  ...
                  <a href={WHATS_HREF} target="_blank" rel="noreferrer">
                    {WHATS_DISPLAY}
                  </a>
                </div>
              </li>
...
.contact-hero h1 {
  font-family:'Bodoni Moda',serif; font-size:clamp(1.6rem,2.4vw,2.2rem);
...
.info-card,
.form-card {
  background:#fff; border:1px solid rgba(28,44,60,.12);
...
.info-icon {
  ...
  border-radius:50%; background:var(--accent); color:#fff; line-height:0;
}
...
.info-list a, .info-list span {
  ...
  color:rgba(28,44,60,.9); text-decoration:none;
}
.info-list a:hover { text-decoration:underline; }

.alert.ok {
  background:var(--beige); border-left:4px solid var(--accent);
...
.field label {
  display:block; font-size:.82rem; margin-bottom:4px; font-weight:600;
}
.field input,
.field textarea {
  width:100%; background:#fff; border:1px solid rgba(28,44,60,.22);
  border-radius:6px; padding:6px 8px;
  font-family:'Montserrat',sans-serif; font-size:.88rem; color:var(--fg);
  outline:none; transition: box-shadow .12s ease, border-color .12s ease;
}
.field input { height:26px; line-height:1.2; }
.field textarea { min-height:72px; line-height:1.35; resize:vertical; }
.field input:focus,
...
.btn {
  ...
  height:32px; padding:0 14px; font-weight:700;
  border-radius:6px; border:1px solid var(--fg);
